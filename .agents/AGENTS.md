...
The first Oracle watch log showed two successful 15-minute polls. The second poll reopened Google's account chooser and completed automatic account selection without MFA, then fetched Course Finder successfully. No ntfy notification was expected because automatic reauthentication is not a failure or MFA event; ntfy currently alerts only on rendered MFA prompts, failed worker states, and recovery. ArchersHub also displays an inactivity countdown after idle time; the portal's measured timeout is now handled by the worker's native keepalive rather than synthetic user input.

Watch mode performs a five-minute ArchersHub keepalive after a successful worker cycle. It calls the portal's own `POST /StudentLogin/ReFillSession/` endpoint inside the authenticated Chrome page context and updates the portal's `localStorage["IdleTime"]` marker, preventing the measured 10-minute inactivity warning and subsequent 2-minute logout without simulating user input. It does not follow returned URLs, enter Google credentials, approve MFA, export cookies, or publish data. Keepalive errors use the normal authentication/provider state and ntfy notification path.

The successful `ReFillSession` response is HTTP 200 HTML for `/StudentDashboard`, not JSON. Accept this authenticated dashboard response; reject login HTML, unexpected HTML destinations, and non-2xx responses. The captured Oracle log showed the former keepalive implementation misclassifying two valid dashboard responses as authentication failures; the response classifier and regression tests cover this case.

The worker waits for Course Finder `networkidle` before classifying a page as unauthenticated; checking immediately after `domcontentloaded` can falsely start the Google flow on every watch cycle while authenticated navigation is still rendering. Its five-minute keepalive also dispatches a non-visible `mousemove` DOM event because ArchersHub's inactivity plugin resets an internal timer from input events; updating `localStorage["IdleTime"]` alone does not reset that timer. This does not move the OS cursor or interact with Google.
