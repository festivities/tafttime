...
Watch mode performs a four-minute ArchersHub keepalive after a successful worker cycle. It sends a tiny Playwright mouse move inside the authenticated Chrome page, updates the portal's `localStorage["IdleTime"]` marker, and calls the portal's own `POST /StudentLogin/ReFillSession/` endpoint, preventing the measured 10-minute inactivity warning and subsequent 2-minute logout. This is browser-page input, not physical OS cursor movement. It does not follow returned URLs, enter Google credentials, approve MFA, export cookies, or publish data. Keepalive errors use the normal authentication/provider state and ntfy notification path.
