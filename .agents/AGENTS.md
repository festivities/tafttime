...
When snapshot publication is implemented on the Oracle host, use a local worker-owned directory rather than adding S3/MinIO: directory mode `0700`, file mode `0600`, and atomic `latest.json` plus one automatically replaced `previous.json`. Keep the last successful current-term data during provider/authentication failures. Always expose its retrieval time; after 30 minutes without a successful refresh, show `Course data may be outdated. Last updated <time>.` If no valid data has ever been published, report temporary unavailability instead of returning an empty catalog.
