...
Status: implemented in September 2026, not yet live-verified. `apps/datapuller/src/lib/archershub-project.ts` projects each offering into Berkeley-shaped `terms`/`courses`/`classes`/`sections`/`enrollmentHistories` documents, reusing the existing `buildCatalogClasses` materialization for `catalog_classes`. Mapping: semester tokens `Term1`/`Term2`/`Term3` (GraphQL `Semester` enum extended in `packages/gql-typedefs` `common.ts` + `term.ts`, backends/frontend regenerated); term names `"<startYear> Term<n>"`; scoped IDs (`archershub-<campus>-<session>` terms, `archershub:<campus>:<session>:<course>` courses); one class per selectable section with a single primary section (no fabricated primary/secondary hierarchy); course code as both subject and course number (frontend dedupes display); enrollment status `O` only when signed availability is positive, `C` when zero/negative, omitted when unknown; Lecture/Laboratory mapped to LEC/LAB; online modality mapped to instruction mode `O`; grading basis, final exam, descriptions, and unparsed meetings omitted rather than fabricated; `academicCareerCode`/`academicCareer` default `UGRD` only to satisfy inherited required filters (career remains unconfirmed); term `retrievedAt` records the latest successful refresh.

```sh
npm run archershub:project --workspace=datapuller -- \
  --mongodb-uri 'mongodb://localhost:3008/tafttime_dlsu_dev?directConnection=true'
```

The command projects all offerings (or one with `--course <id>`), replaces only the affected scopes, rebuilds `catalog_classes` per term without transactions (plain delete + batched insert, so it works on standalone MongoDB), refuses to replace a term's catalog rows when zero classes build, and exits nonzero when any offering is skipped. Ratings and reviews need no code change: creation resolves the projected class identity and aggregates by the scoped `courseId`, and the DLSU dataset starts empty. GradTrak routes and navigation links are removed while `apps/frontend/src/app/GradTrak/` code is retained.

Acceptance (offline): projection shape/idempotency/scope-separation tests; backend/frontend/backend type-checks, lints, and unit tests green; persisted operations regenerated and `check:operations` clean. Remaining: run `archershub:project` against the isolated dev database and verify `catalogSearch`/`class`/`term` queries return the STSWENG rows.

## Phase I: Full-Catalog Collection

Status: implemented in September 2026, not yet live-verified. The worker crawls with `npm run start --workspace=archershub-worker -- --all-courses --snapshot-dir <dir> [--delay-ms 2000] [--course-limit <n>] [--resume]`: sequential per-course `GetCFData` on one page, polite delay plus jitter, three attempts with exponential backoff on provider errors, empty class results recorded as failures (never valid empty snapshots), atomic per-course `<courseId>.json` publication keeping `<courseId>.previous.json`, and a `manifest.json` tracking per-course status/attempts/retrieval times. Resume requires a matching campus/session or the crawl aborts with `SCOPE_CHANGED`; authentication loss aborts after saving progress so the operator can reauthenticate and resume. The page reloads between requests roughly every 15 minutes (never mid-request) to shift the server-context deadline. The importer accepts `--input-dir` (skips `manifest.json`/`*.previous.json`, reports per-file failures, exits nonzero on any failure); the projector reads offerings from MongoDB directly, so directory import then projection completes the pipeline.

The 30-minute staleness rule is wired end-to-end: term documents carry `retrievedAt` (latest successful refresh), exposed as `Term.retrievedAt`, and the catalog shows `Course data may be outdated. Last updated <time>.` when older than 30 minutes. Terms without `retrievedAt` (Berkeley data) show no banner. The five-minute watch cadence and twenty-minute reloads are unchanged; a full crawl takes hours and is expected to cross the session-expiry boundary, which is why resume exists. Do not reload during an in-flight page evaluation.

Acceptance (offline): 8 crawl tests (manifest round-trip, scope-change rejection, empty rejection, backoff, retry-then-ok, persistent failure, auth abort with saved progress); directory-import test. Remaining: trial crawl with `--course-limit` on Oracle, then full crawl, `--input-dir` import, and projection into the isolated dev database.

## Phase J: DLSU Catalog And Scheduler UX

Status: implemented in September 2026, not yet browser-verified. Asia/Manila replaces America/Los_Angeles in the schedule week label, calendar export (correct DST-free VTIMEZONE), class overview/exam dates, enrollment tooltips, and capacity-change tooltips; conflict detection was already timezone-free and unchanged. Term ordering arrays and the catalog fallback accept `Term1`/`Term2`/`Term3`; the filter dropdown renders them as `Term 1`–`Term 3`; `formatSemester` does the same. Course headers and cards show the code once when subject equals course number. GradTrak routes and navigation are hidden with code retained for a future verified prerequisite source. Existing empty states (`No Data`, `No Enrollment Data Available`, `Unavailable for term`) cover missing/unparsed schedules without change.

Acceptance (offline): frontend type-check, lint (0 errors), and unit tests (including new staleness tests) green. Remaining: desktop/mobile browser verification against projected DLSU data, term ordering across academic years, hybrid meetings, and a timezone-correct calendar export. No speculative CSB UI or institutional selector is required.
