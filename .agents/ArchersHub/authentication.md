...
The worker now performs a conservative four-minute keepalive in watch mode. It sends a tiny Playwright mouse move inside the browser page, updates the existing `localStorage["IdleTime"]` marker, and calls the portal's own `POST /StudentLogin/ReFillSession/` endpoint within the authenticated browser context. This resets the portal's inactivity plugin without moving the physical mouse or involving Google authentication material. The endpoint response is only inspected for success/authentication failure; any returned navigation URL is not followed automatically. Keepalive failure enters the normal ntfy/state handling path.
