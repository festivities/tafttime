...
The worker performs conservative four-minute client-side activity maintenance in watch mode. It sends a tiny Playwright mouse move inside the browser page and updates the existing `localStorage["IdleTime"]` marker. Together these reset the portal's inactivity plugin without moving the physical mouse or involving Google authentication material.
...
Do not call `POST /StudentLogin/ReFillSession/` proactively. The common bundle comments out its recurring five-minute call and invokes it only from the expiry-warning `Continue` button. Controlled observations and worker logs showed that a proactive request followed a redirect to authenticated dashboard HTML but cleared Course Finder's server-side context: later requests with nonzero page selections returned `{"CampusDrp":[],"SessionDrp":[],"CourseDrp":[]}`. Client activity plus normal Course Finder polling is used instead.

The Course Finder server context has an observed lifetime of approximately 30 minutes that normal five-minute reads and client activity do not extend. After expiry, its POST endpoints return the JSON string `"Object reference not set to an instance of an object."` with HTTP 200 while the stale Course Finder DOM and URL still look authenticated. An unauthenticated Charlotte request reproduced the same 55-byte response. Treat this exact sentinel as `AUTHENTICATION_REQUIRED`, not a provider outage, and use the normal bounded Google reauthentication flow.

An ordinary authenticated Course Finder page reload before expiry shifts the server-context deadline. Watch mode therefore reloads the retained Course Finder tab every 20 minutes, independent of the data polling interval. This stays ten minutes inside the observed boundary without calling undocumented refresh endpoints. If reloading redirects to login or the 55-byte sentinel still appears, normal bounded reauthentication remains the fallback.
